...
In this project, `comment-service` provides comment, post, like, and search capabilities. `comment-operator` is the operator-facing BFF. The important question is:

```text
How does a frontend HTTP request enter comment-operator,
...
`comment-operator` keeps its own copy of the protocol SDK:

```text
comment-operator/api/comment/v1/comment.proto
comment-operator/api/comment/v1/comment.pb.go
comment-operator/api/comment/v1/comment_grpc.pb.go
```

This means:

```text
comment-operator does not need to depend on the comment-service module.
...
```text
1. Frontend calls comment-operator:
...
For `comment-operator`, the construction chain is:
...
On the BFF side, comment-operator holds client code generated from the same proto.
