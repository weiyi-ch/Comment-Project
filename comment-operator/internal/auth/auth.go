...
func ParseOperatorPrincipal(r *http.Request) (Principal, error) {
	...
	if err != nil || userID <= 0 {
		return Principal{}, kerrors.Unauthorized("UNAUTHORIZED", "missing or invalid operator identity")
	}
...
func parseUserID(r *http.Request) (int64, error) {
	...
	token = strings.TrimPrefix(token, "operator:")
	token = strings.TrimPrefix(token, "operator-")
	if token == "" {
