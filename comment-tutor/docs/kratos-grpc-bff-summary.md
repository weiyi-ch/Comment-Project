...
In this project, `comment-service` provides comment, post, like, and search capabilities. `comment-tutor` is the tutor-facing BFF. The important question is:

```text
How does a frontend HTTP request enter comment-tutor,
...
`comment-tutor` keeps its own copy of the protocol SDK:

```text
comment-tutor/api/comment/v1/comment.proto
comment-tutor/api/comment/v1/comment.pb.go
comment-tutor/api/comment/v1/comment_grpc.pb.go
```

This means:

```text
comment-tutor does not need to depend on the comment-service module.
...
```text
1. Frontend calls comment-tutor:
...
For `comment-tutor`, the construction chain is:
...
On the BFF side, comment-tutor holds client code generated from the same proto.
