...
In this project, `comment-service` provides comment, post, like, and search capabilities. `student-bff` is the student-facing BFF. The important question is:

```text
How does a frontend HTTP request enter student-bff,
...
`student-bff` keeps its own copy of the protocol SDK:

```text
student-bff/api/comment/v1/comment.proto
student-bff/api/comment/v1/comment.pb.go
student-bff/api/comment/v1/comment_grpc.pb.go
```

This means:

```text
student-bff does not need to depend on the comment-service module.
...
For `student-bff`, the construction chain is:
...
On the BFF side, student-bff holds client code generated from the same proto.
